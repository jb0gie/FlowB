...
export class AvatarShowcase extends HTMLElement {
  ...
  private render() {
    ...
    header.innerHTML = `
      ...
      <h1>Hey, I'm FlowB</h1>
      <p class="subtitle">Small robot, big personality. Screen face activated.</p>
      <button id="toggle-screen-btn" class="screen-toggle">📺 Toggle Screen Mode</button>
    `;

    // Add toggle button handler after render
    setTimeout(() => {
      const toggleBtn = this.shadowRoot?.getElementById('toggle-screen-btn');
      if (toggleBtn) {
        toggleBtn.addEventListener('click', () => {
          const modelViewer = this.shadowRoot?.querySelector('model-viewer') as any;
          if (modelViewer?.toggleContentMode) {
            const sampleContent = `
              <div style="text-align: center;">
                <div style="font-size: 1.5rem; margin-bottom: 0.5rem;">👋 Hello!</div>
                <div style="font-size: 0.875rem; opacity: 0.9;">I'm FlowB</div>
                <div style="font-size: 0.75rem; opacity: 0.7; margin-top: 0.5rem;">Ready to help</div>
              </div>
            `;
            const isShowing = modelViewer.toggleContentMode(sampleContent);
            toggleBtn.textContent = isShowing ? '👀 Show Eyes' : '📺 Toggle Screen Mode';
          }
        });
      }
    }, 0);
